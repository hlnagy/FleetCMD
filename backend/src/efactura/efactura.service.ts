...
export class EFacturaService {
  ...
  async generateAuthorizeUrl(): Promise<{ url: string; redirectUri: string }> {
    ...
    const redirectUri = cfg.redirectUri || 'http://localhost:3000/efactura';
    const authorizeUrl = `https://logincert.anaf.ro/anaf-oauth2/v1/authorize?response_type=code&client_id=${cfg.clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&token_content_type=jwt`;
